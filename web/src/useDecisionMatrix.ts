/**
 * Phase 7 Gate P7.5 -- the Decision Matrix state of one open Deal.
 *
 * The one comparison surface: every Strategy x Scenario variant of the Deal's
 * hidden Investment, from one backend request (`POST
 * /investments/{id}/decision-matrix`). This hook asks for the package and holds
 * it. It computes no figure, compares no two cells and keeps no second copy of
 * anything the backend decides -- cross-cell figures included (Q22, DC-5).
 *
 * **Explicit.** Nothing runs until the analyst presses Run Decision Matrix (or
 * Refresh Matrix after a run). A Deal with no saved Strategy and no saved
 * Scenario has nothing to compare, and asks for nothing.
 *
 * **Never stale as current (DC-7).** A package records a token of the saved
 * economic state it ran against: the Deal, the save it was run on, and each
 * Strategy's overlays and each Scenario's overrides by id. It is current only
 * while that token still matches and the base has no unsaved edits. A rename or
 * a new description leaves the token unchanged, so labels update without a
 * re-run. The token only detects that saved state moved; the backend fingerprint
 * remains the financial identity.
 */

import { useMemo, useRef, useState } from 'react';
import { analyzeDecisionMatrix } from './api';
...
type RunState =
  ...
  | { status: 'ready'; token: string; report: DecisionMatrixReport }
...
export interface DecisionMatrixState {
  ...
  report: DecisionMatrixReport | null;
  error: string | null;
...
export function useDecisionMatrix({
  ...
  const token = useMemo(
    ...
    [dealId, savedAt, strategies, scenarios],
  );

  const investmentId =
    strategies[0]?.investment_id ?? scenarios[0]?.investment_id ?? null;
  const hasComparison = strategies.length > 0 || scenarios.length > 0;
  const isRunning = state.status === 'running';
  const canRun = dealId !== null && !isDirty && isReady && investmentId !== null && !isRunning;
  ...
  async function run() {
    ...
    try {
      const report = await analyzeDecisionMatrix(investmentId);
