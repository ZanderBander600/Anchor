...
/** The row-group bands, in document order, with the row labels under each. */
function bands(): { title: string; rows: string[] }[] {
  const out: { title: string; rows: string[] }[] = [];
  for (const row of Array.from(statement().querySelectorAll('tbody tr'))) {
    const group = row.querySelector('th[scope="rowgroup"]');
    if (group !== null) {
      out.push({ title: group.textContent ?? '', rows: [] });
      continue;
    }
    const label = row.querySelector('th[scope="row"]')?.textContent ?? '';
    if (out.length > 0) {
      out[out.length - 1].rows.push(label);
    }
  }
  return out;
}
...
function annualExpectations(): [string, string[]][] {
  ...
  return [
    ...
    ['Less: Free Rent', annual.free_rent_by_year.map(formatCurrency)],
    ...
    ['Less: Credit Loss', annual.credit_loss_by_year.map(formatCurrency)],
...
function monthlyExpectations(): [string, string[]][] {
  ...
  return [
    ...
    ['Less: Free Rent', monthly.free_rent.map(formatCurrency)],
    ...
    ['Less: Credit Loss', monthly.credit_loss.map(formatCurrency)],
