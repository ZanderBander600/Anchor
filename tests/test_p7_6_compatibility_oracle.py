...
def _every_row(db: Path) -> dict[str, list[tuple[Any, ...]]]:
    ...
    return {
        table: rows(db, table)
        for table in sorted(table_names(db) - set(P7_6_TABLES) - set(P7_8_TABLES) - set(P7_9_TABLES))
...
def test_the_migration_adds_exactly_five_empty_sidecars_and_rewrites_no_row(legacy: tuple[Path, dict[str, Any]]) -> None:
    ...
    assert _version(db) == 12  # P7.8B and P7.9 Stage 2 migrate the same v9 database on to schema 12
    assert table_names(db) == before_tables | set(P7_6_TABLES) | set(P7_8_TABLES) | set(P7_9_TABLES)
