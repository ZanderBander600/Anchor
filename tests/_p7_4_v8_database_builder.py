"""P7.4 schema-v8 database builder (not a test module; see
``tests/test_p7_4_compatibility_oracle.py``).

Usage: ``python _p7_4_v8_database_builder.py <repo_root> <db_path> <out.json>``

Writes a genuine schema-v8 database with ``<repo_root>/src``'s own store -- the
pre-P7.4 tree ``aa96155``, exported by ``git archive`` -- never a v9 database
...
- on each of those, a hidden Investment with a neutral and an economic Scenario,
  each analysed, so Quick and Detailed variant cache rows exist;
...
and P7.2 / P7.3-visible exchange: the Deal Library, each Deal, its fingerprints
and ``/analyze`` economics, its Scenarios, its Investment, each Scenario's
fingerprint and analysis, and the Scenario target catalog. The P7.4 tree must
...
assert store._SCHEMA_VERSION == 8, f"expected the v8 tree, got {store._SCHEMA_VERSION}"
assert not hasattr(store, "create_strategy_for_deal"), "the baseline tree already has Strategies"
...
# -- A standalone Deal with no P7 structure ----------------------------------
standalone = store.create_deal("Standalone Quick", QUICK, db_path=db_path)
manifest["standalone"] = {  # type: ignore[index]
    "id": standalone.id, "financial_fingerprint": fingerprint_quick_inputs(QUICK, business_plan=BusinessPlan()),
}
...
# -- Scenarios, through the baseline tree's own P7.2 routes --------------------
ECONOMIC = {
...
for mode, entry in manifest["deals"].items():  # type: ignore[union-attr]
    neutral = client.post(f"/deals/{entry['id']}/scenarios", json={"name": "Neutral"}).json()
    economic = client.post(
        f"/deals/{entry['id']}/scenarios",
        json={"name": "Downside", "description": "Wider", "overrides": [{"unit_id": entry["id"], **ECONOMIC[mode]}]},
    ).json()
    ...
    for scenario_id in entry["scenario_ids"]:
        path = f"/investments/{entry['investment_id']}/scenarios/{scenario_id}/analysis"
...
for name, entry in [*manifest["deals"].items(), ("standalone", manifest["standalone"])]:  # type: ignore[union-attr]
    ...
    if "investment_id" not in entry:
        continue
    record("GET", f"/investments/{entry['investment_id']}")
    record("GET", f"/investments/{entry['investment_id']}/scenarios")
    for scenario_id in entry["scenario_ids"]:
        base = f"/investments/{entry['investment_id']}/scenarios/{scenario_id}"
        record("GET", base)
        record("GET", f"{base}/fingerprint")
        analysis = record("POST", f"{base}/analysis")
        assert analysis["cache_status"] == ("bypassed" if mode == "lease_level" else "hit"), (name, analysis["cache_status"])
