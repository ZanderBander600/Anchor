...
def test_engine_package_contains_only_phase_2a_modules() -> None:
    engine_dir = Path(__file__).resolve().parents[1] / "src" / "mini_anchor" / "engine"
    module_names = {path.name for path in engine_dir.glob("*.py")}

    assert module_names == {"__init__.py", "contracts.py", "noi.py", "debt.py"}


def test_engine_contracts_noi_debt_do_not_import_openpyxl() -> None:
