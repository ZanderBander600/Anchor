...
from _p7_2_fixtures import P7_10_TABLES, without_unstated_classification  # type: ignore[import-not-found]
...
def _every_row(db: Path) -> dict[str, list[tuple[Any, ...]]]:
    """Every row of every table that is not a P7 structure table -- legacy and
    P7.2 alike -- in rowid order.

    A later gate's tables are excluded for the same reason P7.4's are: this
    compares the rows a v8 database already held, and a gate that appends empty
    tables must not read as a row that moved. P7.6's five sidecars and P7.8B's
    six Capital Structure tables are each proved empty by the assertions above,
    and by their own gate's oracle. P7.10 Stage 2's sixteen valuation and
    Investment Memo tables join them for the same reason."""

    return {
        table: rows(db, table)
        for table in sorted(
            table_names(db) - set(P7_4_TABLES) - set(P7_6_TABLES) - set(P7_8_TABLES) - set(P7_9_TABLES) - set(AM1_TABLES)
            - set(ASSET_TYPES_1_TABLES) - set(P7_10_TABLES)
        )
...
def test_the_migration_adds_exactly_eight_empty_tables_and_rewrites_no_row(legacy: tuple[Path, dict[str, Any]]) -> None:
    ...
    # P7.6, P7.8B, P7.9 Stage 2, AM1, Asset Types 1 and P7.10 Stage 2 migrate the
    # same v8 database on to schema 15, each purely additively.
    assert _version(db) == 15
    assert table_names(db) == (
        before_tables | set(P7_4_TABLES) | set(P7_6_TABLES) | set(P7_8_TABLES) | set(P7_9_TABLES) | set(AM1_TABLES)
