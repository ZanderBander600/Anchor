...
from _p7_2_fixtures import P7_4_TABLES, rows, table_names  # type: ignore[import-not-found]
...
def _every_row(db: Path) -> dict[str, list[tuple[Any, ...]]]:
    ...
    return {
        table: rows(db, table)
        for table in sorted(table_names(db) - set(P7_4_TABLES))
...
def test_the_migration_adds_exactly_eight_empty_tables_and_rewrites_no_row(legacy: tuple[Path, dict[str, Any]]) -> None:
    ...
    assert table_names(db) == before_tables | set(P7_4_TABLES)
    ...
    for _ in range(3):
        ...
        assert (_version(db), _schema(db), _every_row(db)) == (9, migrated_schema, migrated_rows)
