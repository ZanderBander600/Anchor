"""P7.6 schema-v9 database builder (not a test module; see
``tests/test_p7_6_compatibility_oracle.py``).

Usage: ``python _p7_6_v9_database_builder.py <repo_root> <db_path> <out.json>``

Writes a genuine schema-v9 database with ``<repo_root>/src``'s own store -- the
pre-P7.6 tree ``6cade62``, exported by ``git archive`` -- never a v10 database
with its version lowered. It holds:

- one Deal per operating mode, each with every artifact its mode persists: a
  Business Plan (Quick and Lease-Level), Deal Context, analysis snapshots (Quick
  and Detailed), AI snapshots and a Lease-Level sensitivity snapshot;
- on each of those, a hidden Investment with a neutral and an economic
  Scenario; on the Quick and Lease-Level ones a Strategy as well;
- a Detailed Deal whose hidden Investment holds only a Strategy;
- one standalone Quick Deal with no P7 structure at all.

Every variant is analysed once before recording, so the Quick and Detailed
variant cache rows exist, and the P7.5 Decision Matrix is run once. It then
records, through that same tree's own HTTP app, every legacy, P7.2, P7.4 and
P7.5 exchange: the Deal Library, each Deal, its fingerprints and ``/analyze``
economics, its Scenarios and Strategies, its Investment, every variant's
inputs, fingerprint and analysis, each Decision Matrix, and both target
catalogs. The P7.6 tree must return exactly the same responses over the
migrated database.
...
assert store._SCHEMA_VERSION == 9, f"expected the v9 tree, got {store._SCHEMA_VERSION}"
assert not hasattr(store, "create_visible_investment"), "the baseline tree already has visible Investments"
...
AI = AIAnalysis(
    ...
    deal_story=DealStory(investment_view="Income-led.", key_strengths=("Coverage.",), key_risks=("Exit cap.",), model_gap=None),
)
...
store.update_ai_snapshot(
    ...
    ai_context_fingerprint=fingerprint_ai(analysis_fingerprint=quick_fp, deal_context="Legacy thesis"), db_path=db_path,
)
...
store.update_analysis_snapshot(
    detailed.id,
    dataclasses.asdict(analyze_detailed_acquisition_with_business_plan(DETAILED_TERMS, DETAILED_OPERATING, business_plan=BusinessPlan())),
    financial_input_fingerprint=detailed_fp, db_path=db_path,
)
store.update_ai_snapshot(
    detailed.id, dataclasses.asdict(AI),
    ai_context_fingerprint=fingerprint_ai(analysis_fingerprint=detailed_fp, deal_context=None), db_path=db_path,
)
...
ll_fp = fingerprint_lease_level_inputs(
    LL_TERMS, LL_PROPERTY, LL_SUITES, LL_LEASES, market_leasing=LL_MARKET, operating_inputs=LL_OPERATING, business_plan=PLAN,
)
store.update_ai_snapshot(
    lease_level.id, dataclasses.asdict(AI),
    ai_context_fingerprint=fingerprint_ai(analysis_fingerprint=ll_fp, deal_context="Legacy rent roll"), db_path=db_path,
)
...
store.update_one_way_sensitivity_snapshot(
    lease_level.id,
    {"configuration": {"metric": "equity_multiple", "assumption": "exit_cap_rate", "values": ("6", "7")}, "result": dataclasses.asdict(one_way)},
...
strategy_only = store.create_detailed_deal("Strategy-only Detailed", DETAILED_TERMS, DETAILED_OPERATING, db_path=db_path)
standalone = store.create_deal("Standalone Quick", QUICK, db_path=db_path)
manifest["standalone"] = {"id": standalone.id, "financial_fingerprint": fingerprint_quick_inputs(QUICK, business_plan=BusinessPlan())}  # type: ignore[index]
...
for mode, entry in manifest["deals"].items():  # type: ignore[union-attr]
    neutral = ok("POST", f"/deals/{entry['id']}/scenarios", {"name": "Neutral"})
    economic = ok("POST", f"/deals/{entry['id']}/scenarios", {
        "name": "Downside", "description": "Wider", "overrides": [{"unit_id": entry["id"], **ECONOMIC[mode]}],
    })
    entry["investment_id"] = neutral["investment_id"]
    entry["scenario_ids"] = [neutral["scenario"]["scenario_id"], economic["scenario"]["scenario_id"]]
    entry["strategy_ids"] = []

for mode in ("quick", "lease_level"):
    entry = manifest["deals"][mode]  # type: ignore[index]
    price = (QUICK if mode == "quick" else LL_TERMS).purchase_price
    created = ok("POST", f"/investments/{entry['investment_id']}/strategies", {
        "name": "Bid low", "overlays": [
            {"unit_id": entry["id"], "domain": "acquisition", "content": {"purchase_price": price - 500_000.0, "acquisition_cost_pct": 0.02}},
        ],
    })
    entry["strategy_ids"].append(created["strategy"]["strategy_id"])

created = ok("POST", f"/deals/{strategy_only.id}/strategies", {
    "name": "Hold longer", "overlays": [{"unit_id": strategy_only.id, "domain": "disposition", "content": {"hold_period": 10}}],
})
manifest["strategy_only"] = {  # type: ignore[index]
    "id": strategy_only.id, "investment_id": created["investment_id"], "scenario_ids": [],
    "strategy_ids": [created["strategy"]["strategy_id"]],
}

investments = [*manifest["deals"].values(), manifest["strategy_only"]]  # type: ignore[union-attr]
for entry in investments:
    for strategy_id in ["base", *entry["strategy_ids"]]:
        for scenario_id in ["base", *entry["scenario_ids"]]:
            ok("POST", f"/investments/{entry['investment_id']}/variants/{strategy_id}/{scenario_id}/analysis")
    ok("POST", f"/investments/{entry['investment_id']}/decision-matrix")
...
for name, entry in [*manifest["deals"].items(), ("strategy_only", manifest["strategy_only"]), ("standalone", manifest["standalone"])]:  # type: ignore[union-attr]
    ...
    record("GET", f"/deals/{entry['id']}/scenarios")
    record("GET", f"/deals/{entry['id']}/strategies")
    if "investment_id" not in entry:
        continue
    investment = entry["investment_id"]
    record("GET", f"/investments/{investment}")
    record("GET", f"/investments/{investment}/scenarios")
    record("GET", f"/investments/{investment}/strategies")
    for scenario_id in entry["scenario_ids"]:
        base = f"/investments/{investment}/scenarios/{scenario_id}"
        record("GET", base)
        record("GET", f"{base}/fingerprint")
        record("POST", f"{base}/analysis")
    for strategy_id in entry["strategy_ids"]:
        record("GET", f"/investments/{investment}/strategies/{strategy_id}")
    for strategy_id in ["base", *entry["strategy_ids"]]:
        for scenario_id in ["base", *entry["scenario_ids"]]:
            variant = f"/investments/{investment}/variants/{strategy_id}/{scenario_id}"
            record("GET", f"{variant}/inputs")
            record("GET", f"{variant}/fingerprint")
            record("POST", f"{variant}/analysis")
    record("POST", f"/investments/{investment}/decision-matrix")
