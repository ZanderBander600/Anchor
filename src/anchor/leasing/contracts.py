...
class LeaseOrigin(StrEnum):
    """Whether a lease was supplied by the analyst or generated by the D2
    rollover engine (D0 Section 4.4).

    Every D1 lease is ``IN_PLACE``: D1 models existing contractual leases
    only and creates no successor of any kind. ``SUCCESSOR`` is declared here
    because it is part of the lease vocabulary D0 fixes, and because D2's
    rollover engine must be able to mark what it produces without changing
    this contract.
    """
...
class LeaseLevelPropertyInputs:
    ...
    ``property_area_sf`` is the denominator for physical occupancy (D1.3) and
    for the area invariant ``occupied + vacant == property_area_sf``.
...
class Suite:
    ...
    valid in D1 and contributes zero revenue and its full area to vacancy.
...
class Lease:
    ...
    **``leased_area_sf``** must equal the suite's ``suite_area_sf`` in D1-D3
    ...
    ``origin`` defaults to ``IN_PLACE``: every analyst-supplied lease is
    in-place, and D1 generates no successor.

    The D2/D3 fields ``free_rent_months`` and ``recovery_basis`` are
    deliberately not declared at D1.0, for the same reason as ``Suite``'s
    market-rent overrides.
    """
