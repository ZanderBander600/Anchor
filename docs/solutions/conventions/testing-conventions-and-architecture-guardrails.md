...
**7. Windows/Git-Bash pytest temp-directory friction recurred without ever being fully closed out — Anchor should fix the cause, not repeat the symptom.**

The "Ignore pytest temporary artifacts" commit (local history only, not cited by SHA here since this repo has no PR history to anchor it against) deleted 83 files that had been accidentally committed under a repo-root, now-deleted `.pytest-temp/<test-name>/...` tree (e.g. a since-removed `.pytest-temp/test_all_blank_value_represent0/input.xlsx`) — test fixture files that should have been written to an OS temp directory and never touched by git at all. Two things are worth flagging plainly rather than assumed fixed:
- **The underlying cause was not conclusively identified in this pass.** `pyproject.toml`'s `[tool.pytest.ini_options]` sets no `basetemp` (`pyproject.toml:28-30`), and every test inspected (`tests/test_excel_reader.py`, `tests/test_cli.py`) uses the standard pytest `tmp_path` fixture correctly rather than writing to a hand-rolled path. The most plausible mechanism is environment-dependent: Python's `tempfile` module (which `tmp_path` ultimately relies on) falls back toward the current working directory when `TMPDIR`/`TEMP`/`TMP` aren't resolved in the process environment a test was run under — plausible for a sandboxed tool-execution environment on Windows/Git Bash if an earlier tool version didn't propagate those variables consistently — but this doc did not verify that mechanism against historical tool behavior, so treat it as a working hypothesis, not a confirmed root cause.
- **The fix that shipped was incomplete.** That commit deleted the already-committed artifacts, but the repo's `.gitignore` (checked 2026-08-27) still has no entry for the pytest-temp directory name — so nothing currently prevents the same accidental commit from happening again if the same environment condition recurs. Before or during the Anchor migration: gitignore that path (and any other environment-dependent temp path this recurs under) regardless of whether the root cause is fully understood, and confirm `TMPDIR`/`TEMP` resolve to a real OS temp directory in whatever tool/shell environment Anchor's tests actually run under, on both Windows and any CI platform used.
